...
const causes = [
  ...
  { id: "trunk-alignment", label: "体幹アライメント・姿勢制御不良", hint: "体幹側屈・回旋・前傾、骨盤傾斜、胸郭位置、片脚支持での姿勢保持を確認。", evidence: "臨床推論", evidenceNote: "代償・原因の両方になり得るため、姿勢制御評価で確認。", patternIds: ["duchenne", "trendelenburg", "waddling-gait"], issueIds: ["trunk-lean", "pelvic-drop", "weight-shift-poor"] },
...
const connectionRules = [
  ...
  },
  {
    ...
    issueIds: ["pelvic-drop", "trunk-lean", "weight-shift-poor"],
    label: "骨盤・体幹と単脚支持の連鎖",
...
function renderCompensations() {
  ...
  compensationList.innerHTML = compensations.length
    ...
    `).join("")
    : `<div class="empty-state">体幹後傾、膝過伸展、分回し、骨盤挙上などが選択されると、代償動作の可能性を表示します。</div>`;
}
